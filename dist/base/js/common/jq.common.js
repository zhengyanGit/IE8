...
; (function (M, window) {
  M.extend({
    ...
    interfacePath: function () {
      ...
      // 	loginTransfer = server+'demo/template/basic/loginTransfer.html';
      ...
      return {
        ...
        // capRegUrl:capUrl+'company/noMenu/returnCompCapOyysReg?sysCode=RZ',//注册页面
        capRegUrl: 'http://member.ouyeelf.com/cap/company/noMenu/returncompanyReg?sysCode=RZ',
        loginUrl: server + 'demo/template/basic/login.html',//登录页面
    ...
    timetrans_hms: function (dateString) {// 格式化日期
      {
        if (!dateString || dateString.lenght == 0) { return ''; }
        var date = '';
        if (dateString.toString().indexOf('-') > -1) {
          date = new Date(dateString.replace(/-/g, '/').replace(/T|Z/g, ''));
        } else {
    ...
    ajaxFn: function (options) {// 封装ajax
      ...
      M.ajax({
        ...
        success: function (data, status, xhr) {
          ...
          // if(data.code == "RY00"){
          //     window.location.href=M.interfacePath.server+"demo/template/basic/500.html";
          // };
          // if(xhr.status == 404){
          //     window.location.href=M.interfacePath.server+"demo/template/basic/404.html";
          //     return false;
          ...
          if (settings.noBomb && M('#tken1').length < 1 && M('#tken2').length < 1) {
            if (!jsonResData.success && jsonResData.code) {
              if (jsonResData.code.indexOf('TK') >= 0) {
                ...
                //                                     	window.location.href=M.interfacePath.server+"/demo/template/basic/login.html";
                //                                         return false;
                //                                     }
                //                                     else if(cod==5){
                //                                     	own.logout();
